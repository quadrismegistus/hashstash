"""LevelDBHashStash: LevelDB-backed key-value engine via plyvel."""
import time
...
@pytest.fixture
def stash(tmp_path):
    s = HashStash(engine="leveldb", root_dir=str(tmp_path / "rockscache"))
    s.clear()
    yield s
    s.clear()
...
def test_persists_across_instances(tmp_path):
    ...
    a = HashStash(engine="leveldb", root_dir=root)
    a.clear()
    a["k"] = "durable"
    a.close()  # drop the process-global handle so b reopens from disk
    b = HashStash(engine="leveldb", root_dir=root)
    assert b["k"] == "durable"
    b.clear()


def test_ttl_expiry(tmp_path):
    stash = HashStash(engine="leveldb", root_dir=str(tmp_path / "ttlcache"), ttl=0.2)
    stash.clear()
    stash["k"] = "v"
    assert stash["k"] == "v"
    time.sleep(0.3)
...
def test_leveldb_in_working_engines():
    from hashstash.config import get_working_engines

    assert "leveldb" in get_working_engines()
